...
3. **급등락 해설 경로 실검증** — dry-run 두 번 모두 급등락 0건이라 뉴스 수집 → LLM 해설 →
   렌더 경로가 **실데이터로는 아직 한 번도 돌지 않았다**. 단위 테스트와 step 3의 실호출
   스모크로만 확인됐다. ±5% 종목이 나오는 날 로그를 확인하거나,
   `STOCKS_MOVE_THRESHOLD=0.5` 로 낮춰 dry-run 해 본다.
